...
def sentiment_data_filtering(sentiment_data, user_thresh, feature_thresh):
    ...
    # iteratively filtering users and features, if the data stay unchanged twice, stop
    un_change_count = 0
    ...
    while True:
        ...
        for key, value in user_dict.items():
            if len(value) > (user_thresh - 1):
                valid_user.add(key)
        sentiment_data = [x for x in sentiment_data if x[0]
                          in valid_user]  # remove user with small interactions
        length = len(sentiment_data)
        if length != last_length:
            last_length = length
            un_change_count = 0
...
def get_user_attention_matrix(sentiment_data, user_num, feature_list, max_range=5):
    ...
    """
    user_counting_matrix = np.zeros(
        (user_num, len(feature_list)))  # tij = x if user i mention feature j x times
    for row in sentiment_data:
        user = row[0]
        for fos in row[2:]:
            feature = fos[0]
            user_counting_matrix[user, feature] += 1
    # xij = [1-N], normalized attention matrix
    ...
    for i in range(len(user_counting_matrix)):
        for j in range(len(user_counting_matrix[i])):
            if user_counting_matrix[i, j] == 0:
                ...
                # norm score
                norm_v = 1 + (max_range - 1) * \
                    ((2 / (1 + np.exp(-user_counting_matrix[i, j]))) - 1)
...
def get_item_quality_matrix(sentiment_data, item_num, feature_list, max_range=5):
    ...
    """
    item_counting_matrix = np.zeros((item_num, len(
        feature_list)))  # kij = x if item i's feature j is mentioned x times
    # sij = x if the overall rating is x (sum up)
    item_sentiment_matrix = np.zeros((item_num, len(feature_list)))
    for row in sentiment_data:
        item = row[1]
        for fos in row[2:]:
            feature = fos[0]
    ...
    for i in range(len(item_counting_matrix)):
        for j in range(len(item_counting_matrix[i])):
            if item_counting_matrix[i, j] == 0:
                ...
                # norm score
                norm_v = 1 + ((max_range - 1) /
                              (1 + np.exp(-item_sentiment_matrix[i, j])))
...
def sample_training_pairs(user, training_items, item_set, sample_ratio=10):
    ...
    negative_items = np.random.choice(
        np.array(list(negative_items)), neg_length, replace=False)
    train_pairs = []
...
def feature_filtering(sentiment_data, valid_features):
    ...
    return np.array(cleaned_sentiment_data, dtype=object)
